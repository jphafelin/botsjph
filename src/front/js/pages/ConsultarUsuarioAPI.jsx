...
export const ConsultarUsuarioAPI = () => {
  ...
  function editAdmin(key, user) {
    localStorage.setItem('id_usuario', key);
    return navigate('/consultar_usuario/usuario_actual');
  }
  ...
  const currentItems = csvData
    .filter((item) => {
      ...
      return (
        searchRegex.test(item.usuario) &&
        searchRegex2.test(item.nombre) &&
    ...
    <div className="justify-content-center">
      {token ? (
        <div>
          <nav className="navbar p-1">
            <div className="container-fluid row">
              ...
              <div className="col-8 text-center justify-content-start ">
                <h3>CONSULTAR USUARIO CON API</h3>
              </div>
          ...
          <div id="consultar-titulo" className="justify-content-center text-light text-center border border-dark border-2">C O N S U L T A R</div>
          <div>
            <button
              id="btn-volver"
              className="btn col-lg-1 m-1 justify border border-3 border-dark text-light"
              onClick={(volver) => navigate('/usuario')}
            >
              VOLVER
            </button>
          </div>
              ...
              {currentItems.map((item, key = item.id) => (
                ...
                >
                  ...
                  <div className="col-8 border border-dark text-start">
                    <b className="mx-2">{item.nombre}</b>
                  </div>
